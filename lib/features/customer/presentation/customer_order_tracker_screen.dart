...
class CustomerOrderTrackerScreen extends StatefulWidget {
  // Kalau dari order-success, orderNumber langsung di-pass
...
class _CustomerOrderTrackerScreenState
  ...
  void initState() {
    super.initState();
    // Auto-search kalau ada initialOrderNumber (dari order-success)
  ...
  // Subscribe realtime untuk order yang sedang ditampilkan
  void _subscribeRealtime(String orderId) {
    ...
    _channel = Supabase.instance.client
        ...
        .onPostgresChanges(
          ...
          callback: (payload) {
            // Update order status secara realtime tanpa full reload
  ...
  Future<void> _search() async {
    ...
    // Unsubscribe dari order sebelumnya
    _channel?.unsubscribe();
    ...
    try {
      ...
      if ((orders as List).isEmpty) {
        if (mounted) {
          setState(() {
            _error   = 'Pesanan tidak ditemukan. Periksa kembali nomor pesananmu.';
            _loading = false;
          });
        }
        return;
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (special != null && special.isNotEmpty)
                Padding(
                  ...
                  child: Text('⚡ $special', style: const TextStyle(
                    fontFamily: 'Poppins', fontSize: 11,
      ...
      return Container(
        ...
        child: const Row(
          ...
          children: [
            ...
            SizedBox(width: 6),
            Text('Pesanan dibatalkan', style: TextStyle(
              fontFamily: 'Poppins', fontSize: 12,
